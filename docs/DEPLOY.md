...
- Firebase 키만 넣는다고 항상 통신되는 것은 아닙니다. 저장소의 `database.rules.json` 전체를 Realtime Database 규칙에 적용해야 기존 토리 경로와 미니톡 부가기능 경로가 함께 동작합니다.
...
- 대화방 명단은 기존 토리 Firebase의 `rooms`에서 직접 불러옵니다.
- 전체 대화는 `socialChat`, 개별 대화방 내역은 `socialChatRooms/{roomId}`에서 최근 100개를 직접 구독합니다.
- 최초 대화방 목록과 대화내역을 포함한 모든 화면 조회·실시간 갱신은 Firebase가 담당합니다.
- Google Sheets에는 `mini_talk_*_backup` 모드로 쓰기 전용 백업만 보내며 앱에서 다시 읽지 않습니다.
- 전체 대화 메시지는 `social_chat` 모드로 기존 `소통` 시트에 백업하고, 개별 방 메시지는 `social_chat_room` 모드로 `대화방` 시트에 백업합니다.
- Google Sheets 채팅 백업은 기존 `소통`(전체방)과 `대화방`(그 외 방) 시트만 사용합니다. 앱은 이 시트를 원본으로 읽지 않습니다.
