...
MiniTalk.Economy.Runtime=(()=>{
  ...
  async function requestDelivery({userId,inventoryId,requestId,ownerNickname=""}){
    assertFirebase();let out=null,reason="";await MiniTalk.Realtime.cloudTransaction(`${inventoryPath(userId)}/${itemKey(inventoryId)}`,cur=>{const item=obj(cur);if(!item.id){reason="missing";return undefined}if(item.usedAt||item.deliveryStatus==="completed"){reason="completed";return undefined}if(item.deliveryStatus==="requested"||item.deliveryStatus==="shipping"){out=item;return item}const ts=now(),nick=String(ownerNickname||item.ownerNickname||"").trim();out={...item,deliveryStatus:"requested",deliveryRequestedAt:ts,...(nick?{ownerNickname:nick}:{})};return out});if(reason){const e=new Error(reason==="completed"?"이미 배송이 완료된 상품입니다.":"배송 요청할 상품을 찾을 수 없습니다.");e.code=reason==="completed"?"DELIVERY_ALREADY_COMPLETED":"INVENTORY_ITEM_NOT_FOUND";throw e}await queueInventoryBackup(userId,{txnId:`delivery:${requestId||inventoryId}`,action:"upsert",item:out});return{ok:true,deliveryStatus:out.deliveryStatus,deliveryRequestedAt:out.deliveryRequestedAt,item:out}
  }
  async function requestDeliveryBulk({userId,inventoryIds,requestId,ownerNickname=""}){const ids=[...new Set((inventoryIds||[]).map(String).filter(Boolean))].slice(0,20),items=[];for(const id of ids){const r=await requestDelivery({userId,inventoryId:id,requestId:`${requestId||"bulk"}:${id}`,ownerNickname});items.push(r.item)}return{ok:true,count:items.length,items}}
