...
MiniTalk.Features.Chats=(()=>{
  ...
  function bindEvents(){if(eventsBound)return;eventsBound=true;
    ...
    MiniTalk.Events.on("rt:profiles",profiles=>{MiniTalk.Store.set("profiles",profiles||{});if(MiniTalk.Store.get("route")!=="chats")return;const active=MiniTalk.Store.get("activeRoom");if(active){applyChatHeader(MiniTalk.Store.get("rooms")?.[active]?.title||"대화",[MiniTalk.UI.Dom.el("button",{class:"icon-button subtle",type:"button",text:"⋯","aria-label":"대화방 메뉴",onclick:()=>openRoomMenu(active)})],{back:()=>backToList()});scheduleMessageRender(active)}else{applyChatHeader("모아루",headerListActions());renderList()}});
  ...
  function backToList(){MiniTalk.Store.set("activeRoom",null);MiniTalk.Realtime.unsubscribeMessages?.();applyChatHeader("모아루",headerListActions(),{back:null});renderList()}
