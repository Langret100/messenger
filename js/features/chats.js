...
MiniTalk.Features.Chats=(()=>{
  ...
  function bindEvents(){if(eventsBound)return;eventsBound=true;
    MiniTalk.Events.on("rt:rooms",rooms=>{roomSnapshotReceived=true;const active=MiniTalk.Store.get("activeRoom"),memberRooms=Object.fromEntries(Object.entries(rooms||{}).filter(([,room])=>MiniTalk.Realtime.isRoomMember(room)));MiniTalk.Store.set("rooms",rooms);notifyRoomInvites(rooms);notifyMemberRoomUpdates(memberRooms,active);MiniTalk.Chat.Unread.syncRooms(memberRooms,active);if(!isRenderedChatRoute())return;if(active&&(!rooms?.[active]||!canViewRoom(rooms[active]))){MiniTalk.UI.Shell.closeModal();backToList();MiniTalk.UI.Shell.toast(rooms?.[active]?"대화방에서 나왔습니다.":"대화방이 삭제되었습니다.");return}if(!active)refreshRoomList()});
  ...
  function roomMemberList(room){if(room.id==="global")return Object.values(MiniTalk.Store.get("presence")||{}).filter(Boolean);return Object.values(room.members||{}).filter(member=>member&&!(Number(member.invitedAt||0)>0&&!Number(member.acceptedAt||0)&&String(member.status||"")!=="member"))}
  async function openRoomMenu(roomId){const room=await MiniTalk.Realtime.getRoom(roomId);if(!room){MiniTalk.UI.Shell.closeModal();backToList();return}const D=MiniTalk.UI.Dom,user=MiniTalk.Store.get("user"),owner=room.creator===user?.user_id,members=roomMemberList(room),body=D.el("div",{class:"room-menu modal-stack"});
    ...
    if(owner&&room.id!=="global"){
      const security=D.el("section",{class:"room-section"});
      security.innerHTML=`<strong class="room-section-title">비밀번호</strong><label class="field"><input id="roomPasswordEdit" type="password" minlength="4" maxlength="32" autocomplete="new-password" placeholder="${room.hasPassword?"새 비밀번호 (변경할 때만 입력)":"4자 이상 입력"}"></label><p class="muted modal-note">${room.hasPassword?"새 비밀번호를 입력했을 때만 변경됩니다. 해제는 아래 버튼에서 별도로 확인합니다.":"설정하면 새 멤버가 입장할 때 입력해야 합니다."}</p>`;
      const save=D.el("button",{class:"button secondary compact-button",type:"button",text:room.hasPassword?"비밀번호 변경":"비밀번호 설정"});
      save.onclick=async()=>{const input=security.querySelector("#roomPasswordEdit"),secret=String(input?.value||"").trim();if(!secret){MiniTalk.UI.Shell.toast(room.hasPassword?"새 비밀번호를 입력하세요. 해제하려면 '비밀번호 해제'를 이용하세요.":"비밀번호를 입력하세요.");input?.focus();return}save.disabled=true;try{await MiniTalk.Realtime.updateRoomPassword(roomId,secret);MiniTalk.UI.Shell.toast("비밀번호 설정을 저장했습니다.");await openRoomMenu(roomId)}catch(e){MiniTalk.UI.Shell.toast(e.message)}finally{save.disabled=false}};
      security.append(save);
      if(room.hasPassword){const clear=D.el("button",{class:"button secondary compact-button",type:"button",text:"비밀번호 해제"});clear.onclick=()=>confirmClearRoomPassword(room);security.append(clear)}
  ...
  function confirmClearRoomPassword(room){const D=MiniTalk.UI.Dom,body=D.el("div",{class:"modal-stack"},[D.el("p",{text:`${room.title||"대화방"}의 비밀번호를 해제할까요?`}),D.el("p",{class:"muted modal-note",text:"해제하면 새 사용자가 비밀번호 없이 참여할 수 있습니다."})]),row=D.el("div",{class:"button-row"}),cancel=D.el("button",{class:"button secondary",type:"button",text:"취소"}),clear=D.el("button",{class:"button room-leave-button",type:"button",text:"비밀번호 해제"});cancel.onclick=()=>openRoomMenu(room.id);clear.onclick=async()=>{clear.disabled=true;try{await MiniTalk.Realtime.clearRoomPassword(room.id);MiniTalk.UI.Shell.toast("비밀번호를 해제했습니다.");await openRoomMenu(room.id)}catch(e){MiniTalk.UI.Shell.toast(e.message);clear.disabled=false}};row.append(cancel,clear);body.append(row);MiniTalk.UI.Shell.modal("비밀번호 해제",body)}
  function inviteRoomDialog(room){
    const D=MiniTalk.UI.Dom,user=MiniTalk.Store.get("user")||{},members=room.members||{},people=new Map();
    // 친구 초대 후보는 로그인 시트 기반의 공통 UserDirectory만 사용합니다.
    // Firebase profile/presence 원본을 다시 합치면 같은 실제 계정이 다른 내부 키로 중복 표시될 수 있습니다.
    (MiniTalk.UserDirectory?.all?.()||[]).forEach(value=>{if(!value||typeof value!=="object"||MiniTalk.UserDirectory?.isGuest?.(value))return;const id=String(value.user_id||"").trim(),nickname=String(value.nickname||id).trim();if(!id||id===String(user.user_id||"")||(members[id]&&!(Number(members[id].invitedAt||0)>0&&!Number(members[id].acceptedAt||0)&&String(members[id].status||"")!=="member"))||/^guest-/i.test(id))return;people.set(id,{...value,user_id:id,nickname,avatar:String(value.avatar||"").trim()})});
  ...
  function buildComposer(roomId){
    ...
    addAction("⌁","파일",async()=>{const result=await MiniTalk.Chat.Attachments.files({
      ...
      onReady:async(payload,{token})=>{try{if(token?.id)await MiniTalk.Realtime.updateMessage(roomId,token.id,{fileUrl:null,fileRef:payload.fileRef,fileName:payload.fileName,fileMime:payload.fileMime,fileSize:payload.fileSize,text:payload.text,uploadState:"ready"});else await sendPayload(roomId,payload)}catch(error){if(payload.fileRef)await MiniTalk.Realtime.removeChatFile(payload.fileRef).catch(()=>{});throw error}},
      onFail:async(error,{file,token})=>{if(token?.id)await MiniTalk.Realtime.updateMessage(roomId,token.id,{text:`[파일 업로드 실패] ${file.name}`,fileUrl:null,fileRef:null,uploadState:"failed"})}
  ...
  async function sendPayload(roomId,payload){return MiniTalk.Realtime.sendMessage(roomId,payload)}
  function messageNode(message){
    ...
    if(type==="game"){
      ...
      bubble.classList.add("file-bubble");const state=String(message.uploadState||"");if(state==="pending"){bubble.append(D.el("div",{class:"file-card file-upload-pending","aria-live":"polite"},[D.el("span",{text:"⌁"}),D.el("span",{},[D.el("strong",{text:message.fileName||"첨부 파일"}),D.el("small",{text:"업로드 중…"})])]))}else if(state==="failed"||(!message.fileRef&&!message.fileUrl)){bubble.append(D.el("div",{class:"file-card file-upload-failed"},[D.el("span",{text:"!"}),D.el("span",{},[D.el("strong",{text:message.fileName||"첨부 파일"}),D.el("small",{text:"업로드 실패"})])]))}else if(message.fileRef){const b=D.el("button",{type:"button",class:"file-card",title:"파일 받기"},[D.el("span",{text:"⌁"}),D.el("span",{},[D.el("strong",{text:message.fileName||"첨부 파일"}),D.el("small",{text:"파일 받기"})])]);b.onclick=async()=>{if(b.disabled)return;b.disabled=true;try{await MiniTalk.Realtime.downloadChatFile(message.fileRef,message.fileName||"file")}catch(error){MiniTalk.UI.Shell.toast(error?.message||"파일을 받지 못했습니다.")}finally{b.disabled=false}};bubble.append(b)}else{const a=D.el("a",{href:message.fileUrl,target:"_blank",rel:"noopener noreferrer",class:"file-card"},[D.el("span",{text:"⌁"}),D.el("span",{},[D.el("strong",{text:message.fileName||"첨부 파일"}),D.el("small",{text:"파일 열기"})])]);bubble.append(a)}
    }else{
