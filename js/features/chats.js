...
MiniTalk.Features.Chats=(()=>{
  ...
  async function openRoom(roomId,verifiedRoom=null){
    /* 목록/로컬 캐시는 표시용입니다. 입장 때는 같은 계정이라도 항상 실제 Firebase room을 다시 읽어
       방 존재·현재 멤버십·비밀번호 상태를 권위 있게 확인합니다. 방금 join에 성공한 객체만 즉시 재사용합니다. */
    let room=verifiedRoom||await MiniTalk.Realtime.getRoom(roomId);
    if(!room){const rooms={...(MiniTalk.Store.get("rooms")||{})};delete rooms[roomId];MiniTalk.Store.set("rooms",rooms);refreshRoomList();MiniTalk.UI.Shell.toast("대화방을 찾을 수 없습니다.");return}
    if(!MiniTalk.Realtime.isRoomMember(room)&&!isAdmin()){if(room.hasPassword){joinRoomDialog(room);return}try{room=await MiniTalk.Realtime.joinRoom(roomId)}catch(e){MiniTalk.UI.Shell.toast(e.message);return}}
    MiniTalk.Store.set("rooms",{...(MiniTalk.Store.get("rooms")||{}),[roomId]:room});MiniTalk.Store.set("activeRoom",roomId);MiniTalk.Store.set("lastRoom",roomId);MiniTalk.Realtime.stopRoomListSubscription?.();MiniTalk.Chat.Unread.clear(roomId,roomMessageTime(room));applyChatHeader(room.title,roomHeaderActions(roomId),{back:()=>backToList()});renderMessages(roomId);MiniTalk.Realtime.subscribeMessages(roomId).catch(error=>{console.warn("대화내역 구독 시작 실패",error);MiniTalk.UI.Shell.toast("대화내역을 불러오지 못했습니다.")})
  }
