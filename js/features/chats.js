...
MiniTalk.Features.Chats=(()=>{
  ...
  function roomHasVisibleActivity(room){return Boolean(String(room?.lastMessage||"").trim())||Number(room?.lastMessageAt||room?.last_message_at||0)>0||Number(room?.messageCount||room?.message_count||room?.messagesCount||0)>0}
  ...
  function renderList(host=MiniTalk.UI.Dom.byId("viewHost")){
    ...
    allRooms.forEach((room,i)=>{const node=roomItem(room);node.dataset.roomSignature=roomRenderSignature(room);node.style.setProperty("--stagger",`${Math.min(i,8)*22}ms`);list.append(node)});
  ...
  function roomRenderSignature(room){
    return JSON.stringify([room?.id||"",room?.title||room?.name||"",room?.lastMessage||"",room?.lastMessageEmoticon||null,roomMessageTime(room),Boolean(room?.hasPassword),Boolean(canViewRoom(room)),Boolean(isFavorite(room?.id)),MiniTalk.Chat.Unread.count(room?.id),room?.type||"group",roomAvatar(room)])
  }
  function refreshRoomList(){
    ...
    let empty=D.one("[data-filter-empty]",list);if(!empty){empty=D.el("div",{class:"empty-state filter-empty hidden","data-filter-empty":"1"},[D.el("span",{text:"●"}),D.el("strong",{text:"대화방이 없습니다"}),D.el("small",{class:"muted",text:"오른쪽 위 ＋ 버튼으로 새 대화를 만들 수 있어요."})]);list.append(empty)}
    const existing=new Map(D.all(".conversation-item[data-room-id]",list).map(node=>[String(node.dataset.roomId||""),node])),wanted=new Set();
    for(const room of allRooms){
      const id=String(room.id||"");if(!id)continue;wanted.add(id);const signature=roomRenderSignature(room);let node=existing.get(id);
      if(!node||node.dataset.roomSignature!==signature){const next=roomItem(room);next.dataset.roomSignature=signature;if(node)node.replaceWith(next);else list.insertBefore(next,empty);node=next}
      list.insertBefore(node,empty)
    }
    for(const [id,node] of existing){if(!wanted.has(id))node.remove()}
    const strong=D.one("strong",empty),small=D.one("small",empty);if(strong)strong.textContent=allRooms.length?"표시할 대화방이 없습니다":"대화방이 없습니다";if(small)small.textContent=allRooms.length?"그룹 탭에서 참여할 대화방을 찾아보세요.":"오른쪽 위 ＋ 버튼으로 새 대화를 만들 수 있어요.";
    filter(query,list,mode,view);list.scrollTop=scrollTop;markRoomListReady(view);
  }
