...
MiniTalk.Features.Feed=(()=>{
  ...
  let state={posts:{}},postsUnsub=null,totalUnsub=null,observer=null,totalHearts=0;
  ...
  function patchPost(id){if(MiniTalk.Router.current()!=="feed")return;const list=MiniTalk.UI.Dom.one(".feed-list"),post=state.posts[id];if(!list)return;if(!post){list.querySelector(`[data-post-id="${CSS.escape(String(id))}"]`)?.remove();syncFeedEmpty(list);return}const next=postCard(post),current=list.querySelector(`[data-post-id="${CSS.escape(String(id))}"]`);current?current.replaceWith(next):list.append(next);sortFeedCards(list);syncFeedEmpty(list);setupLazyMedia(list)}
  ...
  function ensureSub(){
    if(!postsUnsub){state.posts={};postsUnsub=MiniTalk.Realtime.cloudSubscribeChildren(POSTS_PATH,{added:(id,value)=>{state.posts[id]=value;patchPost(id)},changed:(id,value)=>{state.posts[id]=value;patchPost(id)},removed:id=>{delete state.posts[id];patchPost(id)}})}
  ...
  function stopSub(){postsUnsub?.();postsUnsub=null;totalUnsub?.();totalUnsub=null;observer?.disconnect?.();observer=null}
