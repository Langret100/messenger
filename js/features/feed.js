...
MiniTalk.Features.Feed=(()=>{
  ...
  async function toggleHeart(post,button){
    ...
    /* 클릭 제스처가 살아 있을 때 즉시 피드백을 시작합니다.
       서버 응답 뒤에 AudioContext를 만들면 브라우저 자동재생 정책에 막히거나
       실시간 렌더가 버튼을 먼저 갱신해 효과가 눈에 안 보일 수 있습니다. */
  ...
  function playHeartFeedback(button,on,sound=true){
    ...
    if(on){for(let i=0;i<5;i++){const p=MiniTalk.UI.Dom.el("i",{class:"heart-particle",text:"♥"});p.style.setProperty("--dx",`${(i-2)*11}px`);p.style.setProperty("--dy",`${Math.abs(i-2)*2}px`);button.append(p);setTimeout(()=>p.remove(),620)}}
