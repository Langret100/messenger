...
MiniTalk.Features.Feed=(()=>{
  const STATE_PATH="moaru/v3/feedState",POSTS_PATH=`${STATE_PATH}/posts`,TOTALS_PATH=`${STATE_PATH}/totals`,MEDIA_PATH="moaru/v3/feedMedia",MAX_POSTS=40,PHOTO_LIMIT=100*1024,VIDEO_LIMIT=350*1024,VIDEO_SECONDS=7,CLEANUP_KEY="feed.pendingMediaCleanup";
  let state={posts:{}},postsUnsub=null,totalUnsub=null,observer=null,totalHearts=0,renderQueued=false;
  ...
  function scheduleRender(){if(renderQueued||MiniTalk.Router.current()!=="feed")return;renderQueued=true;requestAnimationFrame(()=>{renderQueued=false;if(MiniTalk.Router.current()==="feed")render(MiniTalk.UI.Dom.byId("viewHost"))})}
  function ensureSub(){
    if(!postsUnsub){state.posts={};postsUnsub=MiniTalk.Realtime.cloudSubscribeChildren(POSTS_PATH,{added:(id,value)=>{state.posts[id]=value;scheduleRender()},changed:(id,value)=>{state.posts[id]=value;scheduleRender()},removed:id=>{delete state.posts[id];scheduleRender()}})}
    if(!totalUnsub){const uid=safeUserKey(user().user_id);totalUnsub=MiniTalk.Realtime.cloudSubscribe(`${TOTALS_PATH}/${uid}`,value=>{totalHearts=Math.max(0,Number(value)||0);scheduleRender()})}
  }
  function stopSub(){postsUnsub?.();postsUnsub=null;totalUnsub?.();totalUnsub=null;observer?.disconnect?.();observer=null;renderQueued=false}
  ...
  async function createPost(text,media){
    const u=user(),id=crypto.randomUUID(),profile=MiniTalk.Store.get("profiles")?.[u.user_id]||{},meta={id,user_id:u.user_id,nickname:u.nickname,avatar:profile.avatar||"",text:String(text||"").slice(0,180),mediaType:media?.type||"",createdAt:Date.now(),heartCount:0,hearts:{}};
    ...
    try{await MiniTalk.Realtime.cloudSet(`${POSTS_PATH}/${id}`,meta)}catch(error){if(media?.data){try{await MiniTalk.Realtime.cloudRemove(`${MEDIA_PATH}/${id}`)}catch{queueCleanup(id)}}throw error}
    ...
    try{
      ...
      while(rows.length>MAX_POSTS){const old=rows.shift();if(!old?.id)continue;await MiniTalk.Realtime.cloudRemove(`${POSTS_PATH}/${old.id}`);try{await MiniTalk.Realtime.cloudRemove(`${MEDIA_PATH}/${old.id}`)}catch{queueCleanup(old.id)}}
    }catch(error){console.warn("오래된 피드 정리 실패",error)}
  }
  async function toggleHeart(post){
    ...
    const saved=await MiniTalk.Realtime.cloudTransaction(postPath,current=>{if(!current)return current;const next=structuredClone(current),hearts=next.hearts||{},on=hearts[uid]===true;delta=on?-1:1;if(on)delete hearts[uid];else hearts[uid]=true;next.hearts=hearts;next.heartCount=Math.max(0,(Number(next.heartCount)||0)+delta);return next});
    ...
    try{await MiniTalk.Realtime.cloudTransaction(`${TOTALS_PATH}/${authorKey}`,current=>Math.max(0,(Number(current)||0)+delta))}
      ...
      await MiniTalk.Realtime.cloudTransaction(postPath,current=>{if(!current)return current;const next=structuredClone(current),hearts=next.hearts||{},on=hearts[uid]===true;if(delta>0&&on){delete hearts[uid];next.heartCount=Math.max(0,(Number(next.heartCount)||0)-1)}else if(delta<0&&!on){hearts[uid]=true;next.heartCount=(Number(next.heartCount)||0)+1}next.hearts=hearts;return next}).catch(()=>{});throw error
    }
  }

  function render(host){if(!host)return;ensureSub();const D=MiniTalk.UI.Dom,u=user(),view=D.el("section",{class:"view feed-view view-enter"}),head=D.el("section",{class:"feed-head section-card"},[D.el("div",{class:"feed-head-main"},[D.el("strong",{text:"우리 피드"})]),D.el("div",{class:"feed-heart-total","aria-label":`받은 하트 ${totalHearts}개`},[D.el("span",{text:"♥"}),D.el("b",{text:String(totalHearts)})])]),list=D.el("div",{class:"feed-list"});postRows().forEach(post=>list.append(postCard(post)));if(!postRows().length)list.append(D.el("div",{class:"empty-state"},[D.el("span",{text:"♡"}),D.el("strong",{text:"아직 게시물이 없어요"}),D.el("small",{class:"muted",text:"짧은 글과 사진·영상을 올려보세요."})]));view.append(head,list);if(!u.isGuest)view.append(D.el("button",{class:"feed-fab",type:"button","aria-label":"피드 올리기",text:"＋",onclick:compose}));host.replaceChildren(view);setupLazyMedia(view)}
  function postCard(post){const D=MiniTalk.UI.Dom,u=user(),uid=safeUserKey(u.user_id),liked=post.hearts?.[uid]===true,card=D.el("article",{class:"feed-card","data-post-id":post.id}),avatar=D.el("img",{class:"feed-avatar",src:post.avatar||"assets/mascot-avatar.png",alt:""});avatar.onerror=()=>{avatar.src="assets/mascot-avatar.png"};card.append(D.el("header",{class:"feed-card-head"},[avatar,D.el("div",{},[D.el("strong",{text:post.nickname||"사용자"}),D.el("small",{class:"muted",text:new Date(Number(post.createdAt)||Date.now()).toLocaleString("ko-KR")})]) ]));if(post.text)card.append(D.el("p",{class:"feed-text",text:post.text}));if(post.mediaType){const mediaHost=D.el("div",{class:`feed-media-placeholder${post.mediaType==="video"?" video-gated":""}`,"data-media-id":post.id,"data-media-type":post.mediaType},[D.el("span",{text:post.mediaType==="video"?"▶ 영상 보기":"사진 불러오는 중…"})]);if(post.mediaType==="video")mediaHost.onclick=()=>{mediaHost.onclick=null;mediaHost.classList.add("loading");mediaHost.replaceChildren(D.el("span",{text:"영상 불러오는 중…"}));loadMedia(mediaHost)};card.append(mediaHost)};const heart=D.el("button",{class:`feed-heart ${liked?"active":""}`,type:"button","aria-label":"하트",onclick:()=>toggleHeart(post)},[D.el("span",{text:liked?"♥":"♡"}),D.el("b",{text:String(Number(post.heartCount)||0)})]);card.append(D.el("footer",{class:"feed-card-foot"},[heart]));return card}
  function setupLazyMedia(root){observer?.disconnect?.();if(typeof IntersectionObserver==="undefined"){root.querySelectorAll?.('[data-media-type="image"]').forEach(loadMedia);return}observer=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting){observer.unobserve(e.target);loadMedia(e.target)}}),{rootMargin:"220px"});root.querySelectorAll?.('[data-media-type="image"]').forEach(el=>observer.observe(el))}
  async function loadMedia(host){const id=host.dataset.mediaId;try{const media=await MiniTalk.Realtime.cloudGet(`${MEDIA_PATH}/${id}`,null);if(!host.isConnected)return;if(!media?.data)return host.replaceChildren(MiniTalk.UI.Dom.el("span",{class:"muted",text:"미디어가 없습니다."}));host.replaceChildren(media.type==="video"?MiniTalk.UI.Dom.el("video",{src:media.data,controls:true,playsinline:true,preload:"metadata"}):MiniTalk.UI.Dom.el("img",{src:media.data,alt:"게시물 사진",loading:"lazy"}))}catch{host.replaceChildren(MiniTalk.UI.Dom.el("span",{class:"muted",text:"미디어를 불러오지 못했습니다."}))}}
  return{id:"feed",title:"피드",icon:"♡",render,leave:stopSub};
})();
