...
MiniTalk.Features.MoaChat=(()=>{
  ...
  async function render(host){
    ...
    const starterRows=MiniTalk.AI.MoaCommunicationEngine.starterSuggestions?.()||[["날씨","서울 오늘 날씨 알려줘"],["심심해","심심해 ㅋㅋ"]].map(([label,prompt])=>({label,prompt}));
    starterRows.forEach(({label,prompt})=>{const chip=Dom.el("button",{class:"moa-suggestion-chip",type:"button",text:label,"aria-label":prompt,"data-no-drag-scroll":"true"});chip.onclick=()=>{input.value=prompt;form.requestSubmit?.()||send.click()};suggestions.append(chip)});
    ...
    form.append(input,send);zone.append(hint);zone.append(suggestions);zone.append(form);view.append(listNode,zone);host.replaceChildren(view);setTimeout(()=>input.focus(),30)
  }
