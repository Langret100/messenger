...
MiniTalk.Features.MoaChat=(()=>{
  ...
  function appendRichText(node,text){
    ...
    parts.forEach(part=>{if(/^https?:\/\//i.test(part)){const a=doc.createElement("a");a.href=part;a.target="_blank";a.rel="noopener noreferrer";a.className="moa-answer-link";a.textContent=part;node.append(a)}else node.append(doc.createTextNode(part))});
  }
