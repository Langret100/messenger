...
MiniTalk.Features.Settings=(()=>{
  ...
  function openAdminUnlock(){
    ...
    submit.onclick=async()=>{submit.disabled=true;const reserved=MiniTalk.Features.Admin?.reservePopup?.(MiniTalk.UI.Dom.byId("viewHost"))===true;const directoryWarm=MiniTalk.UserDirectory?.loaded?.()?Promise.resolve():Promise.resolve(MiniTalk.UserDirectory?.refresh?.()).catch(()=>null);try{const role=await MiniTalk.AdminSession.unlock(input.value);MiniTalk.UI.Shell.closeModal();MiniTalk.UI.Shell.renderNav();MiniTalk.UI.Shell.toast(role==="SHOP_MANAGER"?"쇼핑몰 관리자 권한이 활성화되었습니다.":"전체 관리자 권한이 활성화되었습니다.");await Promise.race([directoryWarm,new Promise(resolve=>setTimeout(resolve,1200))]);await MiniTalk.Router.go("admin")}catch(error){if(reserved)MiniTalk.Features.Admin?.closePopup?.();MiniTalk.UI.Shell.toast(error.message||"관리자 인증에 실패했습니다.");input.select();submit.disabled=false}};
