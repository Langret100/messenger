...
MiniTalk.Features.Settings=(()=>{
  ...
  function render(host){
    ...
    const admin=D.el("button",{class:"settings-row",type:"button",disabled:Boolean(user.isGuest),onclick:()=>MiniTalk.AdminSession.authorized()?MiniTalk.Router.go("admin"):openAdminUnlock()},[D.el("span",{class:"settings-row-icon",text:"◆"}),D.el("span",{class:"settings-row-copy"},[D.el("strong",{text:"관리자 권한"}),D.el("small",{class:"muted",text:user.isGuest?"로그인 계정에서만 이용 가능":MiniTalk.AdminSession.authorized()?"인증됨 · 관리자 기능 열기":"고유 코드로 관리자 인증"})]),D.el("span",{class:"row-arrow",text:user.isGuest?"":"›"})]);
    const about=D.el("div",{class:"settings-row static-row"},[D.el("span",{class:"settings-row-icon",text:"i"}),D.el("span",{class:"settings-row-copy"},[D.el("strong",{text:"미니톡"}),D.el("small",{class:"muted",text:`버전 ${MiniTalkConfig.version}`})])]);
  ...
  function openAdminUnlock(){
    const D=MiniTalk.UI.Dom,body=D.el("div",{class:"modal-stack"}),input=D.el("input",{type:"password",maxlength:"80",autocomplete:"off",placeholder:"관리자 고유 코드"}),submit=D.el("button",{class:"button primary",type:"button",text:"관리자 인증"});
    submit.onclick=async()=>{submit.disabled=true;try{await MiniTalk.AdminSession.unlock(input.value);MiniTalk.UI.Shell.closeModal();MiniTalk.UI.Shell.renderNav();MiniTalk.UI.Shell.toast("관리자 권한이 활성화되었습니다.");await MiniTalk.Router.go("admin")}catch(error){MiniTalk.UI.Shell.toast(error.message||"관리자 인증에 실패했습니다.");input.select();submit.disabled=false}};
