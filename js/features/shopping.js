...
MiniTalk.Features.Shopping = (() => {
  ...
  function deliveryAdminPanel(context = {}) {
      ...
      grouped.forEach(groupRows => {
        ...
        groupComplete.onclick=async()=>{
          ...
          try{
            ...
            draw();Shell.toast(`${savedCount}건을 이 사용자 배송완료로 처리했습니다.`);
          }catch(error){
    ...
    const load = async () => {
      ...
      try { MiniTalk.AdminSession.requireToken("SHOP");rows=MiniTalk.Realtime?.getMode?.()==="firebase"&&MiniTalk.Economy.Runtime?await MiniTalk.Economy.Runtime.deliveryList():await MiniTalk.AuthApi.shopDeliveryList(current.user_id, MiniTalk.AdminSession.requireToken("SHOP"));const makePeople=()=>new Map((MiniTalk.UserDirectory?.all?.({includeSelf:true})||[]).map(p=>[String(p.user_id),p]));let people=makePeople();const unresolved=()=>rows.some(row=>{const owner=String(row.ownerId||row.owner_id||"");return owner&&!row.nickname&&!row.ownerNickname&&!people.get(owner)?.nickname});if(unresolved()&&MiniTalk.UserDirectory?.refresh){try{await MiniTalk.UserDirectory.refresh();people=makePeople()}catch(error){console.warn("배송 요청자 닉네임 명단 갱신 실패",error)}}rows=rows.map(row=>{const owner=String(row.ownerId||row.owner_id||""),nickname=String(row.nickname||row.ownerNickname||people.get(owner)?.nickname||"").trim();return{...row,ownerId:owner,owner_id:owner,nickname:nickname||owner}});draw(); }
