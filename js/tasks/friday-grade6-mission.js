...
MiniTalk.Tasks.FridayGrade6Mission=(()=>{
  ...
  function render(){const D=MiniTalk.UI.Dom,info=windowInfo(),card=D.el("section",{class:`friday-mission-card section-card ${info.open?"open":"locked"}`},[D.el("div",{class:"friday-mission-icon",text:info.open?"✓":"🔒"}),D.el("div",{class:"friday-mission-copy"},[D.el("strong",{text:"금요일 초6 학습점검"}),D.el("small",{class:"muted",text:info.open?"수학·국어 20문항 · 오늘 자정 전까지":"평소에는 잠겨 있어요"}),D.el("b",{class:"friday-countdown",text:info.open?"지금 풀 수 있어요":`열리기까지 ${remainLabel()}`})]),D.el("button",{class:"button primary compact-button",type:"button",text:info.open?"시작":"잠김",disabled:!info.open||user().isGuest,onclick:open})]);if(!info.open){const label=card.querySelector(".friday-countdown"),timer=setInterval(()=>{if(!card.isConnected)return clearInterval(timer);const next=windowInfo();if(next.open){clearInterval(timer);if(MiniTalk.Store.get("route")==="tasks")MiniTalk.Features.Tasks.render(MiniTalk.UI.Dom.byId("viewHost"));return}label.textContent=`열리기까지 ${remainLabel()}`},30000)}return card}
