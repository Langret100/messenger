...
MiniTalk.Tools.ClassInfo=(()=>{
  ...
  async function openTimetable(){const D=MiniTalk.UI.Dom,body=D.el("div",{class:"class-info modal-stack"}),data=await MiniTalk.Realtime.cloudGet(TIMETABLE,null),preview=D.el("div",{class:"class-info-preview"});if(data?.image)preview.append(D.el("img",{src:data.image,alt:"오늘의 시간표"}));else preview.append(D.el("p",{class:"muted",text:"등록된 시간표가 없습니다."}));const upload=D.el("button",{class:"button primary",type:"button",text:"시간표 이미지 바꾸기"});upload.onclick=async()=>{if(MiniTalk.Store.get("user")?.isGuest)return MiniTalk.UI.Shell.toast("로그인 후 수정할 수 있습니다.");const file=await pick("image/*");if(!file)return;upload.disabled=true;try{const image=await compressImage(file);await MiniTalk.Realtime.cloudSet(TIMETABLE,{image,updatedAt:nowMs(),updatedBy:MiniTalk.Store.get("user")?.nickname||""});MiniTalk.UI.Shell.closeModal();MiniTalk.UI.Shell.toast("시간표를 갱신했습니다.")}catch(e){MiniTalk.UI.Shell.toast(e.message);upload.disabled=false}};body.append(preview,upload);MiniTalk.UI.Shell.modal("오늘의 시간표",body)}
