/* ============================================================
   ...
   ============================================================ */
MiniTalk.UI.Shell=(()=>{
  const D=doc=>doc?MiniTalk.UI.Dom.forDocument(doc):MiniTalk.UI.Dom;let started=false,entering=false,activeUserId=null,loadingCount=0,loadingTimer=0,loadingHost=null,navSyncTimer=0;const modalCloseTimers=new WeakMap();
  ...
  function scheduleNavSync(delay=0){clearTimeout(navSyncTimer);navSyncTimer=setTimeout(()=>{navSyncTimer=0;renderNav()},Math.max(0,Number(delay)||0))}
  function ensurePrimaryNav(){renderNav();requestAnimationFrame(()=>renderNav());[80,260,900].forEach(ms=>setTimeout(()=>renderNav(),ms))}
