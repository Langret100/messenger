...
ok(admin.includes('if(coin){refreshBalances(')&&admin.includes('scheduleBalanceRefresh()'),'coin balance is not lazy/background refreshed only in coin mode');
...
// 변경 에셋 캐시 버전 동기화
for(const ref of ['js/adapters/realtime.js?v=64.5.51','js/shopping/store-service.js?v=64.5.25','js/tasks/task-service.js?v=64.5.26','js/features/admin.js?v=64.5.41'])ok(index.includes(ref),'cache ref missing '+ref);
