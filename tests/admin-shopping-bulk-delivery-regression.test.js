...
ok(/shop_request_delivery_bulk[\s\S]{0,220}\}, 10000\)/.test(auth),'customer bulk delivery is not capped to 10 seconds');
ok(/shop_delivery_complete_bulk[\s\S]{0,220}\}, 10000\)/.test(auth),'admin bulk completion is not capped to 10 seconds');
ok(/admin_task_list[\s\S]{0,180}\}, 10000\)/.test(auth),'interactive admin requests are not capped to 10 seconds');
