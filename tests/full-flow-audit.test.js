...
ok(admin.includes('reloadQueued=true')&&admin.includes('mode="submitted";const load=')&&admin.includes('value===mode?"active"'), 'automatic task refresh can be lost or reset the selected review tab');
...
ok(server.includes('userId = requireKnownMoaruUser_(p.user_id)')&&server.includes('targets.map(String).filter(function (id, index, list)')&&!server.includes('return targets.map(String).filter'), 'command polling still scans the coin ledger for every user');
...
for(const ref of ['js/core/user-directory.js?v=64.5.8','js/adapters/auth-api.js?v=64.5.8','js/adapters/realtime.js?v=64.5.18','js/economy/coin-wallet.js?v=64.5.8','js/shopping/store-service.js?v=64.5.10','js/tasks/task-service.js?v=64.5.10','js/tasks/task-window.js?v=64.5.8','js/features/shopping.js?v=64.5.9','js/features/admin.js?v=64.5.10','js/app.js?v=64.5.19'])ok(html.includes(ref),`stale audited asset version: ${ref}`);
