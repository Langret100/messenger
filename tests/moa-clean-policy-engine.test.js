...
ok(html.includes('moa-communication-engine.js?v=29'),'v89 cache bust missing');
ok(sw.includes('moaru-moa-admin-isolation-url-filter-v10'),'v89 service worker cache missing');
...
(async()=>{const e=sandbox.MiniTalk.AI.MoaCommunicationEngine;await e.sync(true);
 ...
 const snap=e.debugSnapshot();ok(snap.version===92&&snap.state.phase,'v89 state/version missing');
