...
ok(html.includes('moa-communication-engine.js?v=17'),'v89 cache bust missing');
ok(sw.includes('moaru-camera-popup-task-scroll-fix'),'v89 service worker cache missing');
...
(async()=>{const e=sandbox.MiniTalk.AI.MoaCommunicationEngine;await e.sync(true);
 ...
 ok(feedback.every(x=>x.type==='policy_feedback')&&feedback.every(x=>!('trigger' in x)&&!('reply' in x)),'raw/personal feedback leaked');
