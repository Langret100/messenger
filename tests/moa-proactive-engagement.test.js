...
ok(html.includes('moa-chat.css?v=6')&&html.includes('moa-communication-engine.js?v=29')&&html.includes('moa-chat.js?v=14'),'v90 cache bust missing');
ok(sw.includes('moaru-moa-admin-isolation-url-filter-v10'),'v90 service worker cache missing');
...
(async()=>{const e=sandbox.MiniTalk.AI.MoaCommunicationEngine;await e.sync(true);
 ...
 let snap=e.debugSnapshot();ok(snap.version===92&&snap.state.openLoops.length>=1&&typeof snap.profile.initiative==='number','v90 engagement state missing');
