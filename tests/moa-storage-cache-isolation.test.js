...
ok(auth.includes('const data = await post({ mode: "admin_task_list", user_id: userId, admin_token: adminToken }, 10000);'),'interactive admin task list is not capped to 10 seconds');
...
ok(sw.includes('moaru-runtime-bundle-'),'service-worker cache namespace not bumped');
ok(app.includes('sw.js?v='),'service-worker registration version not bumped');
ok(index.includes('js/app.js?v='),'app cache reference not bumped');
