...
ok(html.includes('js/ai/moa-chat-engine.js?v=11'),'v81 engine cache bust missing');
ok(sw.includes('moaru-v64.5.48-moa-holistic-v84-20260820'),'v81 service worker cache missing');
...
(async()=>{
 const r1=await E.reply('오늘 학교에서 축구했어'); ok(r1.source==='dialogue_core'&&/축구|그다음|어떻게/.test(r1.reply),'general sports statement not handled by dialogue core: '+r1.reply);
 ...
 const r6=await E.reply('근데 마지막에 졌어');ok(/아쉽|힘들|빡|괜찮/.test(r6.reply),'loss statement not handled');
