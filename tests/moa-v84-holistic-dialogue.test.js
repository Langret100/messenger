...
ok(html.includes('js/ai/moa-dialogue-core.js?v=2'),'dialogue core not loaded');
ok(html.indexOf('moa-dialogue-core.js')<html.indexOf('moa-chat-engine.js'),'dialogue core must load before engine');
ok(sw.includes('moaru-v64.5.48-moa-conversation-v85-20260820'),'v84 cache version missing');
...
(async()=>{
  let r=await E.reply('오늘 스케이트보드 탔어');ok(r.source==='dialogue_core'&&!/조금만 더 알려줘|에 대해 조금만/.test(r.reply),'unknown hobby topic not handled: '+r.reply);
  r=await E.reply('킥플립 연습했어');ok(r.source==='dialogue_core'&&!/조금만 더 알려줘|에 대해 조금만/.test(r.reply),'new niche topic not handled: '+r.reply);
  r=await E.reply('드디어 성공했어');ok(/잘됐|뿌듯|성공|좋았|해냈|기분/.test(r.reply),'success continuation not handled: '+r.reply);
  ...
  r=await E.reply('수학 문제 하나 겨우 풀었어');ok(r.source==='dialogue_core'&&!/조금만 더 알려줘|에 대해 조금만/.test(r.reply),'academic statement not generic');
  r=await E.reply('우리 강아지가 장난감 숨겼어');ok(r.source==='dialogue_core'&&!/조금만 더 알려줘|에 대해 조금만/.test(r.reply),'pet story not generic: '+r.reply);
