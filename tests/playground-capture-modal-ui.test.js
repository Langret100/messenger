...
ok(html.includes('js/features/chats.js?v=64.5.3'), 'stale cache version for js/features/chats.js');
ok(html.includes('js/features/auth.js?v=64.5.1'),'stale cache version for js/features/auth.js');
ok(html.includes('js/app.js?v=64.5.4'),'stale cache version for js/app.js');
ok(html.includes('css/app.css?v=64.5.4'),'stale cache version for css/app.css');
ok(html.includes('js/ui/shell.js?v=64.5.3'),'stale cache version for js/ui/shell.js');
for(const asset of ['js/tools/profile-editor.js','js/tools/capture.js','js/features/tools.js'])ok(html.includes(`${asset}?v=64.4`),`stale cache version for ${asset}`);
ok(sw.includes('moaru-v64.5.4-shopping-nav-polish-20260818')&&app.includes('sw.js?v=64.5.4'),'service worker v64.5.4 cache update is missing');
...
ok(css.includes(':root[data-window-mode="standalone"] .workspace{grid-template-columns:1fr;grid-template-rows:1fr 34px}')&&css.includes('grid-template-rows:1fr calc(34px + env(safe-area-inset-bottom,0px))'),'bottom navigation remains excessively tall');
