...
ok(css.includes('.modal-host{align-items:center;padding:14px}')&&css.includes('.header-create-button{font-size:23px!important')&&css.includes('.header-search-button::before{content:"⌕"')&&css.includes('transform:translate(-50%,-59%)'),'mobile modal centering or header icon sizing is missing');
...
ok(html.includes('js/features/chats.js?v=64.5'), 'stale cache version for js/features/chats.js');
ok(html.includes('js/features/auth.js?v=64.5.1'),'stale cache version for js/features/auth.js');
ok(html.includes('js/app.js?v=64.5.2'),'stale cache version for js/app.js');
ok(html.includes('css/app.css?v=64.5.1'),'stale cache version for css/app.css');
for(const asset of ['js/ui/shell.js','js/tools/profile-editor.js','js/tools/capture.js','js/features/tools.js'])ok(html.includes(`${asset}?v=64.4`),`stale cache version for ${asset}`);
ok(sw.includes('moaru-v64.5.2-final-audited-20260818')&&app.includes('sw.js?v=64.5.2'),'service worker v64.5.2 cache update is missing');
