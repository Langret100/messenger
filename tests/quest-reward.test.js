...
const ctx={console,EventTarget,Event,CustomEvent:CE,window:null,document:{},URLSearchParams,setTimeout,clearTimeout,fetch:async(url,options)=>{requests.push({url,body:String(options.body)});return{ok:true,json:async()=>({ok:true,granted:true})}},localStorage:{getItem:k=>memory.has(k)?memory.get(k):null,setItem:(k,v)=>memory.set(k,String(v)),removeItem:k=>memory.delete(k)}};
...
ctx.MiniTalk.Economy={CoinWallet:{refresh:async()=>2}};
ctx.MiniTalk.UI={Shell:{toast(){}}};
...
(async()=>{
  ...
  console.log('QUEST_REWARD_OK',requests.length,'subject rewards');
