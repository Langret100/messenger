...
if(!realtime.includes('setInterval(pollServerCommands,60000)'))throw new Error("Apps Script command safety polling was not restored");
if(!tasks.includes('pollTimer')||!tasks.includes('setInterval(() => refresh(true).catch(() => {}), 120000)'))throw new Error("Apps Script task safety polling was not restored");
...
if(!admin.includes('const fallback=setInterval')||!admin.includes('120000')||!admin.includes('tasks:admin-refresh'))throw new Error("admin task review must keep signal plus Apps Script safety refresh");
...
for(const ref of ['realtime.js?v=64.5.52','task-service.js?v=64.5.27','features/tasks.js?v=64.5.4','features/admin.js?v=64.5.42','app.js?v=64.5.46'])if(!index.includes(ref))throw new Error(`cache-busted signal-idle asset missing: ${ref}`);
