...
const ctx={console,REWARD_SHEET:"보상",COL_REWARD_USER_ID:1,COL_REWARD_COIN:3,MANUAL_WEB_APP_URL:"https://example.test/exec",SpreadsheetApp:{openById:()=>workbook},ContentService:{MimeType:{JSON:"json"},createTextOutput:value=>({value,setMimeType(){return this}})},LockService:{getScriptLock:()=>({tryLock:()=>true,releaseLock(){}})},CacheService:{getScriptCache:()=>({get:()=>null,put(){},remove(){}})},PropertiesService:{getScriptProperties:()=>({getProperty:key=>props.get(key)||null,setProperty:(key,value)=>props.set(key,String(value)),setProperties:values=>Object.entries(values).forEach(([key,value])=>props.set(key,String(value))),deleteProperty:key=>props.delete(key),getProperties:()=>Object.fromEntries(props)})},Utilities:{getUuid:()=>"command-id",formatDate:(date)=>date.toISOString()},getRewardUserData_:rewardData,processCoinChangeUnlocked_:(id,action,amount)=>{const data=rewardData(id);if(!data)return{success:false};const before=Number(reward.rows[data.rowIndex-1][2])||0,next=action==="add"?before+amount:before-amount;if(next<0)throw new Error("잔액 부족!");reward.rows[data.rowIndex-1][2]=next;return{success:true,newCoin:next}}};
vm.createContext(ctx);vm.runInContext(fs.readFileSync(path.join(root,"docs/apps-script/Code.gs"),"utf8"),ctx);const coin=fs.readFileSync(path.join(root,"docs/apps-script/coin.gs"),"utf8"),coinAmountHelper=coin.match(/function requireCoinAmount_\(raw\) \{[\s\S]*?\n\}/);if(!coinAmountHelper)throw new Error("coin amount helper missing");vm.runInContext(coinAmountHelper[0],ctx,{filename:"coin.gs#requireCoinAmount_"});vm.runInContext(fs.readFileSync(path.join(root,"docs/apps-script/coin-shopping-extension.gs"),"utf8"),ctx);
// This test covers signup + admin coin operations, not token cryptography. Production token behavior has dedicated admin-session/reliability tests.
ctx.requireAdminToken_=(_userId,token)=>String(token||"")==="token"?{ok:true,role:"ADMIN"}:{ok:false,error:"ADMIN_SESSION_EXPIRED"};
ctx.requireShopManagerToken_=(_userId,token)=>String(token||"")==="token"?{ok:true,role:"ADMIN"}:{ok:false,error:"ADMIN_SESSION_EXPIRED"};
...
const idempotentParams={user_id:"admin-user",admin_token:"token",targets_json:'["U-new"]',amount:"4",reason:"중복 방지",request_id:"same-coin-request"};
const firstIdempotent=parse(ctx.handleAdminCoinReward({parameter:idempotentParams})),secondIdempotent=parse(ctx.handleAdminCoinReward({parameter:idempotentParams}));
if(!firstIdempotent.ok||!secondIdempotent.ok||rewardData("U-new").coin!==7)throw new Error("retried administrator adjustment changed the balance twice");
