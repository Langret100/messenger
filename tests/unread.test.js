...
U.syncRooms({a:{id:'a',updatedAt:100}},null); if(U.count('a')!==0)throw new Error('initial room should not count unread');
U.syncRooms({a:{id:'a',updatedAt:200}},null); if(U.count('a')!==1)throw new Error('room update should count unread');
U.syncRooms({a:{id:'a',updatedAt:300}},'a'); if(U.count('a')!==1)throw new Error('active room should not add unread');
U.clear('a',300); if(U.count('a')!==0)throw new Error('clear failed');ctx.MiniTalk.Store.set('user',{user_id:'user-b'});U.syncRooms({a:{id:'a',updatedAt:400}},null);if(U.count('a')!==0)throw new Error('unread state leaked across users');ctx.MiniTalk.Store.set('user',{user_id:'user-a'});if(U.count('a')!==0)throw new Error('original user unread state corrupted');
