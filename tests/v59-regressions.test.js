...
if(realtime.indexOf("MiniTalkConfig.paths.legacyProfiles")>realtime.indexOf("if(firebaseAuthenticated){"))throw new Error("legacy profiles must be subscribed before the Firebase-auth branch");
if(!realtime.includes("publishProfiles();emit(\"shop-inventory\""))throw new Error("unauthenticated Firebase mode clears persistent profiles");
